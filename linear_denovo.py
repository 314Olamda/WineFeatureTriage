...
def _all_compositions(
    ...
    IMPORTANT: `residues` must be pre-sorted by MASS ascending (see
    compositions_for_linear_mass), not alphabetically as in the original
    WineCycloPep version. Alphabetical order has no relationship to mass,
    so a "stop once mass exceeds target" break was unsafe there. With
    mass-ascending order, once one residue's mass pushes the running total
    past target+tol, every subsequent residue (heavier) will too, so the
    loop can `break` immediately. Combined with an upper-bound prune (can
    the remaining slots, filled with the heaviest available residue, even
    reach target?), this cut real-world runtime at 700-1500 Da queries from
    2-16 seconds down to well under 0.1s.
    """
    n = len(current)
    ...
    max_residue_mass = residues[-1][1] if residues else 0.0
    remaining_slots = n_max - n
    if current_mass + remaining_slots * max_residue_mass < target_mass - tol:
        return

    for aa, mass_val in residues:
        if mass_val < last_mass - 1e-9:
            continue
        ...
        _all_compositions(target_mass, residues, n_min, n_max, tol,
                           current, new_mass, results, last_mass=mass_val,
...
def compositions_for_linear_mass(
    ...
    """
    Find amino acid compositions whose linear neutral mass (sum of
    residues + H2O) matches precursor_neutral_mass within tol.

    max_results caps enumeration and stops the search early once reached.
    This matters because mid-range masses (roughly 700-1500 Da) can have
    THOUSANDS of distinct amino-acid compositions matching within 0.02 Da
    -- a genuine characteristic of de novo composition search with the
    full 20-residue alphabet and no additional constraints, not a search
    bug. Composition mass alone cannot resolve this; fragment-ion scoring
    (score_spectrum_vs_linear) is what actually disambiguates among
    candidates, so exhaustively enumerating every mass-matching composition
    before scoring wastes time without adding identification power. Pass
    max_results=1 for a fast existence check (e.g. classification triage);
    leave at the default for real candidate generation.
    """
    ...
    if allowed_aa:
        ...
        residues = list(RESIDUE_MASS.items())
    residues.sort(key=lambda x: (x[1], x[0]))
    results: list[list[str]] = []
...
EXACT_ENUM_MAX_LEN = 8  # above this, full itertools.permutations() enumeration
                        # (9! = 362880, 10! = 3628800) is too slow to run once
                        # per candidate -- switch to direct random sampling


def all_permutations_linear(composition: str, max_perms: int | None = None) -> list[str]:
    """
    Unique orderings of a composition. Unlike the cyclic case, there is no
    rotational equivalence to collapse — linear IAA, AAI, AIA are three
    genuinely distinct peptides with different b/y ladders.

    For length <= EXACT_ENUM_MAX_LEN: exact enumeration, deduplicated.
    Above that: samples up to max_perms permutations directly via random
    shuffling rather than building the full factorial space first.
    """
    n = len(composition)
    if n <= EXACT_ENUM_MAX_LEN:
        seen: set[str] = set()
        out: list[str] = []
    ...
    target_n = max_perms if max_perms else 200
    seen: set[str] = set()
    chars = list(composition)
    attempts, max_attempts = 0, target_n * 20
    while len(seen) < target_n and attempts < max_attempts:
...
def generate_decoys_linear(sequence: str, n_decoys: int = 100) -> list[str]:
    ...
    if n <= EXACT_ENUM_MAX_LEN:
        ...
        all_perms = set()
        chars = list(sequence)
        attempts, max_attempts = 0, n_decoys * 20
        while len(all_perms) < n_decoys and attempts < max_attempts:
            random.shuffle(chars)
            all_perms.add("".join(chars))
            attempts += 1
